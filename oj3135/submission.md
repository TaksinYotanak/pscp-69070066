...
```text
โจทย์ให้ฉันเช็คว่ามีกี่คนที่ได้ตรวจสอบของขวัญถ้าหากของขวัญถูกโจรขโมยการตรวจสอบถือว่าสิ้นสุด

Input:
รับค่าจำนวนคนทั้งหมด จำนวนคนที่ส่งถัดไปและหมายเลขโจร

Output:
จำนวนคนที่ได้ตรวจสอบของขวัญ

Constraints:
input เป็นจำนวนเต็มสามจำนวน N K T (1 <= N <= 200,000; 1 <= K <= N-1; 1 <= T <= N)
```
...
```text
Step 1: รับจำนวนเต็ม 3 ค่า
Step 2: ทำให้มันloop วนในจำนวนคนในห้อง
Step 3: ถ้้าเกิดกลับมาที่ 1 ให้หยุดเพราะหมายความว่า loop ครบแล้วนับจำนวนคนที่ได้ตรวจสอบ
Step 4: ถ้าเกิดเลขไปอยู่ที่เลขที่เป็นขโมยให้หยุดloop
```
...
```text
ต่างจากวิธีแรกฉันย้ายบรรทัดการเก็บจำนวนคนที่ได้ตรวจสอบของขวัญมาไว้ข้างล่างเงื่อนไขถ้าเกิดลูปครบแล้วให้หยุดเพื่อแก้ปัญหาการเพิ่มจำนวนคนตรวจสอบซ้ำในคนแรก
```
...
```text
เช็คว่าจำนวนคนที่ได้ตรวจสอบถูกต้อง
```
...
```text
ถ้าเกิดในห้องมี 1 คนควรจะได้เช็คแค่ 1 คน
```
...
```text
ถ้าเกิดให้ค่าที่เก็บจำนวนคนตรวจสอบเป็น 0 output ควรจะผิด
```
