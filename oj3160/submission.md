...
```text
โจทย์ให้ฉันหาจำนวนเฉพาะระหว่าง a - b

Input:
รับจำนวนค่า a ถึง b

Output:
จำนวนเฉพาะและจำนวนของจำนวนเฉพาะ

Constraints:
output ต้องเป็นจำนวนเฉพาะเท่านั้น
```
...
Step 2: loop ซ้อนลูปและตรวจสอบว่าลูปแรก % ลูปที่ 2 แล้วค่าเป็น 0 มั้ย 
Step 3: ถ้าจริงให้หยุดและลูปเลขต่อไป
Step 4: ถ้าไม่จริงให้นับและเก็บเลขนั้นไว้
...
```text
ต่างจากแผนแรกคือฉันเช็คก่อนว่าค่าที่ลูปแรกที่รับมามากกว่า 2  มั้ยเพราะว่าจำนวนเฉพาะจะเป็นจริงก็ต่อเมื่อ 1 กับตัวมันเองหารลงตัว ซึ่งเรารู้อยุ่แล้วว่า 1 หารลงทุกตัว
```
...
```text
เช็คว่าสามารถเช็คจำนวนเฉพาะที่ถูกต้องได้มั้ย
```
...
```text
เช็คว่าถ้าเกิดไม่ใช้ join  output ควรจะติดกัน
```
...
```text
ถ้าไม่มีเงื่อนไข > 2 output ควรจะมี 1
```
