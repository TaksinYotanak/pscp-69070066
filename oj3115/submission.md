...
```text
โจทย์ให้เช็คว่าในแต่ละเวลาที่กำหนดมีกี่ร้านที่เปิดอยู่

Input:
โปรแกรมรับจำนวนร้าน และช่วงเวลา

Output:
นับจำนวนร้านที่เปิดอยู่ในแต่ละเวลาที่กำหนดมา

Constraints:
ข้อมูลเป็นจํานวนเต็มทั้งหมด,จํานวนร้านค้ามีได้มากสุดแสนร้าน,จํานวนเวลาที่ต้องตรวจสอบได้มากสุดแสนครั้ง
```
...
```text
Step 1: รับค่าจำนวนร้านและช่วงเวลา
Step 2: สร้าง list ว่างไว้เก็บเวลาเปิด-ปิดของแต่ละร้าน
Step 3: loop และเช็คว่าแต่ละช่วงเวลามีร้านไหนเปิดบ้าง
Step 4: แสดงจำนวนร้านที่เปิดในแต่ละช่วงเวลาเป็นบรรทัดเดียว
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ for loop ในการเช็คว่าเวลาในแต่ละร้านตรงกับเวลาที่กำหนดในช่วงเวลาไหนบ้าง
```
...
```text
เช็คว่าเวลาที่กำหนดตรงกับร้านที่เปิดในเวลานั้น
```
...
```text
ถ้าหากไม่มี end = "" จะไม่โชว์บรรทัดเดียว
```
...
```text
เช็คว่าถ้าหากร้านมีเวลาเปิดไม่ตรงกับเวลาที่กำหนดควรออกมาเป็น 0
```
